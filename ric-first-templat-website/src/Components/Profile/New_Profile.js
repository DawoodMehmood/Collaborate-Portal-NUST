...
import sdg4 from "../../Icons/sdgs/4.jpg";
import sdg5 from "../../Icons/sdgs/5.jpg";
import sdg6 from "../../Icons/sdgs/6.jpg";
import sdg7 from "../../Icons/sdgs/7.jpg";
import sdg8 from "../../Icons/sdgs/8.jpg";
import sdg9 from "../../Icons/sdgs/9.jpg";
import sdg10 from "../../Icons/sdgs/10.jpg";
import sdg11 from "../../Icons/sdgs/11.jpg";
import sdg12 from "../../Icons/sdgs/12.jpg";
import sdg13 from "../../Icons/sdgs/13.jpg";
import sdg14 from "../../Icons/sdgs/14.jpg";
...
const New_Profile = ({
  ...
  const showIntellectualProperties = () => {
    ...
    const patents = Intellectual_Property.Patents
      ? Intellectual_Property.Patents.length
      : 0;
    const industrialDesign = Intellectual_Property.Industrial_Design
      ? Intellectual_Property.Industrial_Design.length
      : 0;
    const copyRights = Intellectual_Property.Copy_Rights
      ? Intellectual_Property.Copy_Rights.length
      : 0;
    const tradeMarks = Intellectual_Property.Trade_Marks
      ? Intellectual_Property.Trade_Marks.length
      : 0;
  ...
  const [CollapseOptions, setOptions] = useState({
    // Options for collapsing sections
  ...
  const [Publications_Pie_Chart_Date, setPublications_Pie_Chart_Date] =
    useState({}); // State Holding data for Publications Pie Chart
    ...
    await fetch(image_URL) // Fetching Image from URL
      .then(async (response) => {
        if (response.status === 200) {
          ...
          setResearch_Images((prevState) => {
            return [...prevState, imageObjectURL];
          }); //Updating State to, which will eventually show images on slider on profile Page.
        } else {
  ...
  const fetchData = async (query) => {
    ...
    await fetch(
      `https://customsearch.googleapis.com/customsearch/v1?key=${API_KEY}&cx=${CSE_ID}&q=${query} Logo&searchType=image`
    )
      .then((response) => {
  ...
  const fetchScopusInfo = async (article_DOI) => {
    ...
    fetch("https://apix.collaborate.nust.edu.pk/api/Author", {
      ...
      .then((data) => {
        ...
        const authors =
          data["abstracts-retrieval-response"]["authors"]["author"];
    ...
    if (type === "National") {
      await setProject_Pie_Chart_Date((prev) => {
        if (prev["National Projects"] === undefined) {
          return { ...prev, "National Projects": 1 };
        }
        return { ...prev, "National Projects": prev["National Projects"] + 1 };
      });
    }
    ...
    // If project type is "International", add it to "International" State Array
      ...
      await setProject_Pie_Chart_Date((prev) => {
        if (prev["International Projects"] === undefined) {
          return { ...prev, "International Projects": 1 };
        }
        return {
          ...prev,
          "International Projects": prev["International Projects"] + 1,
        };
      });
    }
  }
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < values.length; i++) {
      values[i].sort((a, b) =>
        a.Publication_year < b.Publication_year ? 1 : -1
      );
    }
    ...
    if (publications.hasOwnProperty("Article")) {
      ...
      for (let i = article_array.length - 1; i > 0; i--) {
        ...
        if (article_array[i]["Indexation"] === "HEC - W Cat") {
          // If Paper has indexation type of "HEC - W Cat", then proceed
          const DOI = article_array[i]["DOI"]
            .replace("DOI ", "")
            .replace("DOI", "")
            .replace("doi ", "")
            .replace("doi", ""); // Cleaning DOI
          if (DOI.includes("https://") || DOI.includes("http://")) {
            // Separating and creating a new DOI from existing one
    ...
    for (let prop in publications) {
      if (prop === "Data Paper") {
        ...
        UpdatePublicationPiData("Chapter", publications[prop].length).then(
          () => {
        ...
        UpdatePublicationPiData("Article", publications[prop].length).then(
          () => {
            setResearch_Articles((prevState) => [
              ...prevState,
              ...publications[prop],
            ]);
          }
  ...
  useEffect(() => {
    ...
    async function separateProjects() {
      ...
      await projects.map((project) => {
        ...
        if (
          projects_data.hasOwnProperty(project.Approval_Date.substring(0, 4))
        ) {
        ...
        temp_amount =
          temp_amount + Number((project.Cost_in_PKR / 1000000).toFixed(2));
      });
    }
  ...
  const openCV = () => {
    ...
    html2canvas(check, { backgroundColor: null }).then((canvas) => {
      ...
      if (profile[0].e_mail.trim() !== "") {
        ...
        doc.text(`${profile[0].e_mail}`, rect1X + 50, y + 18, {
          maxWidth: 320,
        });
      }
      ...
      if (profile[0].Work_Phone.trim() !== "") {
        ...
        //write code to add text on the right side of this image
        doc.text(`${profile[0].Work_Phone}`, rect1X + 50, y + 10, {
          maxWidth: 320,
        });
      }
      ...
      if (profile[0].linkedin_URL.trim() !== "") {
        ...
        //write code to add text on the right side of this image
        doc.text(`${profile[0].linkedin_URL}`, rect1X + 50, y + 12, {
          maxWidth: 300,
        });
      }
      ...
      if (profile[0].twitter_URL.trim() !== "") {
        ...
        //     write code to add text on the right side of this image
        doc.text(`${profile[0].twitter_URL}`, rect1X + 50, y + 15, {
          maxWidth: 300,
        });
      }
      /*
      ...
      if (profile[0].Experience.length > 0) {
        ...
        if (
          lastFieldHeight >
          doc.getCurrentPageInfo()["pageContext"]["mediaBox"]["topRightY"] - 150
        ) {
        ...
        profile[0].Experience.map((experience, index) => {
          ...
          printing_string = `${experience.org_name}`
            .replace(/^\s*\n/gm, "")
            .trim();
          doc.text(printing_string, rect1X + 55, lastFieldHeight, {
            maxWidth: 450,
          });
          if (doc.splitTextToSize(`${experience.org_name}`, 450).length === 1) {
            lastFieldHeight +=
              doc.splitTextToSize(`${experience.org_name}`, 450).length + 18;
          } else {
            lastFieldHeight +=
              doc.splitTextToSize(`${experience.org_name}`, 450).length * 4 +
              18;
          }
        });
      }
      ...
      if (profile[0].Awards.length > 0) {
        ...
        profile[0].Awards.map((award, index) => {
          ...
          printing_string = `${index + 1}. ${award.Location.replace(
            ":",
            ""
          ).replace("-", "")}`
            .replace(/^\s*\n/gm, "")
      ...
      if (profile[0].KeyNotes.length > 0) {
        ...
        profile[0].KeyNotes.map((keyNotes, index) => {
          ...
          if (doc.splitTextToSize(`${keyNotes.Title}`, 450).length === 1) {
            lastFieldHeight +=
              doc.splitTextToSize(`${keyNotes.Title}`, 450).length + 18;
          } else {
            lastFieldHeight +=
              doc.splitTextToSize(`${keyNotes.Title}`, 450).length * 4 + 18;
          }
        });
      }
      ...
      if (profile[0].Professional_Memberships_Registrations.length > 0) {
        ...
        doc.setFont("helvetica", "normal");
        lastFieldHeight += doc.getTextDimensions(title).h + 5;
        profile[0].Professional_Memberships_Registrations.map(
          (memberships, index) => {
            if (
              lastFieldHeight >
              doc.getCurrentPageInfo()["pageContext"]["mediaBox"]["topRightY"] -
      ...
      if (Project_Research.National.length > 0) {
        ...
        Project_Research.National.map((researchProject, index) => {
          ...
          printing_string = `${index + 1}. ${researchProject.Title}`
            .replace(/^\s*\n/gm, "")
            .trim();
          doc.text(printing_string, heading_x_axis, lastFieldHeight, {
            maxWidth: 450,
          ...
          lastFieldHeight +=
            doc.splitTextToSize(printing_string, 450).length * 12;
          doc.setFontSize(8);
          doc.setTextColor(0, 0, 0);
          ...
          if (doc.splitTextToSize(printing_string, 450).length === 1) {
            lastFieldHeight +=
              doc.splitTextToSize(printing_string, 450).length + 18;
          } else {
            lastFieldHeight +=
              doc.splitTextToSize(printing_string, 450).length * 4 + 18;
          }
          common_index += 1;
        });
      }
      ...
      if (Project_Industry.National.length > 0) {
        ...
        Project_Industry.National.map((industryProject, index) => {
          ...
          printing_string = `${index + 1}. ${industryProject.Title}`
            .replace(/^\s*\n/gm, "")
            .trim();
          doc.text(printing_string, heading_x_axis, lastFieldHeight, {
            maxWidth: 500,
      ...
      if (Research_Articles.length > 0) {
        ...
        Research_Articles.map((article, index) => {
          ...
          doc.setFontSize(11);
          doc.setTextColor(0, 0, 0);
          printing_string = `${index + 1}. ${article.Title}`
            .replace(/^\s*\n/gm, "")
            .trim();
          doc.text(printing_string, heading_x_axis, lastFieldHeight, {
            maxWidth: 500,
          ...
          lastFieldHeight +=
            doc.splitTextToSize(printing_string, 500).length * 12;
          doc.setFontSize(8);
          doc.setTextColor(0, 0, 0);
          printing_string = `Authors: ${article.All_Authors.trim().substring(
            0,
            article.All_Authors.length - 2
          )}`
            .replace(/^\s*\n/gm, "")
            .trim();
          doc.text(`${printing_string}`, 40, lastFieldHeight, {
            maxWidth: 450,
          });
          lastFieldHeight +=
            doc.splitTextToSize(printing_string, 450).length * 11;
          printing_string = `${article.Journal_Title} ${Pub_Info(
      ...
      if (conferences.length > 0) {
        ...
        conferences.map((conference, index) => {
          ...
          doc.setFontSize(11);
          doc.setTextColor(0, 0, 0);
          printing_string = `${index + 1}. ${conference.Title}`
            .replace(/^\s*\n/gm, "")
            .trim();
          doc.text(printing_string, heading_x_axis, lastFieldHeight, {
            maxWidth: 500,
          ...
          doc.setFontSize(8);
          doc.setTextColor(0, 0, 0);
          printing_string = `Authors: ${conference.Authors.join(", ")}`
            .replace(/^\s*\n/gm, "")
            .trim();
          ...
          if (doc.splitTextToSize(printing_string, 500).length === 1) {
            lastFieldHeight +=
              doc.splitTextToSize(printing_string, 500).length + 18;
          } else {
            lastFieldHeight +=
              doc.splitTextToSize(printing_string, 500).length * 4 + 18;
          }
        });
      }
      ...
      if (editorials.length > 0) {
        ...
        editorials.map((editorial, index) => {
          ...
          doc.setFontSize(11);
          doc.setTextColor(0, 0, 0);
          printing_string = `${index + 1}. ${editorial.Title}`
            .replace(/^\s*\n/gm, "")
            .trim();
          doc.text(printing_string, heading_x_axis, lastFieldHeight, {
            maxWidth: 500,
          ...
          doc.setFontSize(8);
          doc.setTextColor(0, 0, 0);
          printing_string = `${editorial.Reviewer_Type}`
            .replace(/^\s*\n/gm, "")
            .trim();
          doc.text(`${editorial.Reviewer_Type}`, rect1X + 40, lastFieldHeight, {
            maxWidth: 500,
          });
          lastFieldHeight +=
            doc.splitTextToSize(printing_string, 500).length * 11;
          ...
          doc.text(printing_string, rect1X + 40, lastFieldHeight, {
            maxWidth: 450,
          });
          if (doc.splitTextToSize(printing_string, 500).length === 1) {
            lastFieldHeight +=
      ...
      if (Book_Chapters.length > 0) {
        ...
        Book_Chapters.map((chapter, index) => {
          ...
          doc.setFontSize(11);
          doc.setTextColor(0, 0, 0);
          printing_string = `${index + 1}. ${chapter.Title}`
            .replace(/^\s*\n/gm, "")
            .trim();
          doc.text(printing_string, heading_x_axis, lastFieldHeight, {
            maxWidth: 500,
          ...
          doc.setFontSize(8);
          doc.setTextColor(0, 0, 0);
          printing_string = `Authors: ${chapter.All_Authors.trim().substring(
            0,
            chapter.All_Authors.length - 2
          )}`
            .replace(/^\s*\n/gm, "")
            .trim();
          doc.text(printing_string, 40, lastFieldHeight, { maxWidth: 500 });
      ...
      if (Books.length > 0) {
        ...
        Books.map((book, index) => {
          ...
          doc.setFontSize(11);
          doc.setTextColor(0, 0, 0);
          printing_string = `${index + 1}. ${book.Title}`
            .replace(/^\s*\n/gm, "")
            .trim();
          doc.text(printing_string, heading_x_axis, lastFieldHeight, {
            maxWidth: 500,
          ...
          doc.setFontSize(8);
          doc.setTextColor(0, 0, 0);
          printing_string = `Authors: ${book.All_Authors.trim().substring(
            0,
            book.All_Authors.length - 2
          )}`
            .replace(/^\s*\n/gm, "")
            .trim();
      ...
      if (Intellectual_Property.Patents.length > 0) {
        ...
        Intellectual_Property.Patents.map((ip, index) => {
          ...
          doc.setFontSize(11);
          doc.setTextColor(0, 0, 0);
          printing_string = `${index + 1}. ${ip.Title}`
            .replace(/^\s*\n/gm, "")
            .trim();
          ...
          doc.setFontSize(8);
          doc.setTextColor(0, 0, 0);
          printing_string = `Authors: ${ip.Inventors.join(", ")}`
            .replace(/^\s*\n/gm, "")
            .trim();
          ...
          lastFieldHeight +=
            doc.splitTextToSize(printing_string, 500).length * 11;
          printing_string = `${ip.Schools.join(", ")}`
            .replace(/^\s*\n/gm, "")
            .trim();
      ...
      if (Intellectual_Property.Industrial_Design.length > 0) {
        ...
        Intellectual_Property.Industrial_Design.map((ip, index) => {
          ...
          printing_string = `Authors: ${ip.Inventors.join(", ")}`
            .replace(/^\s*\n/gm, "")
            .trim();
          doc.text(`Authors: ${ip.Inventors.join(", ")}`, 40, lastFieldHeight, {
            maxWidth: 500,
          });
          lastFieldHeight +=
            doc.splitTextToSize(printing_string, 500).length * 11;
          ...
          printing_string = `${ip.Schools.join(", ")}`
            .replace(/^\s*\n/gm, "")
            .trim();
          doc.text(`${ip.Schools.join(", ")}`, 40, lastFieldHeight, {
            maxWidth: 500,
          });
          if (doc.splitTextToSize(printing_string, 500).length === 1) {
            lastFieldHeight +=
      ...
      if (Intellectual_Property.Trade_Marks.length > 0) {
        ...
        Intellectual_Property.Trade_Marks.map((ip, index) => {
          ...
          printing_string = `${ip.Inventors.join(", ")}`
            .replace(/^\s*\n/gm, "")
            .trim();
          doc.text(printing_string, 40, lastFieldHeight, { maxWidth: 500 });
          lastFieldHeight +=
      ...
      if (Supervision.PHD.length > 0) {
        ...
        Supervision.PHD.map((supervision, index) => {
          ...
          doc.setFontSize(11);
          doc.setTextColor(0, 0, 0);
          printing_string = `${index + 1}. ${supervision.Title}`
            .replace(/^\s*\n/gm, "")
            .trim();
          doc.text(printing_string, heading_x_axis, lastFieldHeight, {
            maxWidth: 500,
  ...
  const Pub_Info = (pub_info, year) => {
    if (pub_info === "NULL" || pub_info === "") {
      ...
      if (values.length > 2) {
        if (
          values[0].trim().includes("Volume ") ||
          values[0].trim().includes("Volume:") ||
          values[0].trim().includes("Vol.")
        ) {
          if (values[0].trim().includes("Volume:")) {
            final_String =
              final_String + `${values[0].trim().split(":")[1]} (${year})`;
          } else {
            final_String =
              final_String + `${values[0].trim().split(" ")[1]} (${year})`;
          }
        ...
        if (
          values[1].trim().includes("Issue ") ||
          values[1].trim().includes("Issue:") ||
          values[1].trim().includes("No.") ||
          values[1].trim().includes("Number")
        ) {
          if (values[1].trim().includes("Issue:")) {
            final_String =
              final_String + ` ${values[1].trim().split(":")[1]}, `;
          } else {
            final_String =
              final_String + ` ${values[1].trim().split(" ")[1]}, `;
          }
        ...
        if (
          values[2].trim().includes("Pages ") ||
          values[2].trim().includes("Pages:") ||
          values[2].trim().includes("Article Number ") ||
          values[2].trim().includes("Article Number:")
        ) {
        ...
        <Card key={index} bg="light" text="dark" style={{ width: "100%" }}>
          ...
          <Card.Body>
            <Card.Subtitle className="mb-2 text-muted">
              {`Authors: ${article.All_Authors.trim().substring(
                0,
                article.All_Authors.length - 2
              )}`}
              <br />
              <i>{`${article.Journal_Title}, `}</i>&nbsp;
              <strong className={"strong-color"}>{`${Pub_Info(
                article.Journal_Info,
                article.Publication_year,
                index
              )}`}</strong>
              ...
              {article.IF === "" ? (
                ...
                <strong className={"strong-color"}>{`${article.IF} `}</strong>
              )}{" "}
              &nbsp; &nbsp; Citations:{" "}
              {article.Citations === "" ? (
                <strong className={"strong-color"}>0</strong>
    ...
    return (
      <>
        <Card key={index} bg="light" text="dark" style={{ width: "100%" }}>
          ...
          <Card.Body>
            <Card.Subtitle className="mb-2 text-muted">
              {`Authors: ${chapter.All_Authors.trim().substring(
                0,
                chapter.All_Authors.length - 2
              )}`}
              <br />
    ...
    return (
      <>
        <Card key={index} bg="light" text="dark" style={{ width: "100%" }}>
          ...
          <Card.Body>
            <Card.Subtitle className="mb-2 text-muted">
              {`${book.Journal_Title} ${"(" + book.Journal_Info + ")"}`}
              <br />
              {`Authors: ${book.All_Authors.trim().substring(
                0,
                book.All_Authors.length - 2
              )}`}
              <br />
  ...
  const Research_international_List = Project_Research.International.map(
    (research, index) => {
      return (
        <>
          <Card key={index} bg="light" text="dark" style={{ width: "100%" }}>
            ...
            <Card.Body>
              <Card.Subtitle className="mb-2 text-muted">
                <span className="bold">
                  {research.Funding_Agency.replace("+", "")}
                </span>
                {`, Rs: ${(research.Cost_in_PKR / 1000000).toFixed(2)}M, `}
                {research["Project_Status"] === "Completed" ? (
                  ...
                  <>
                    <span className="bold">
                      {research["Project_Status"].substring(11)}
                    </span>
                    <span> ( {research["Approval_Date"]} )</span>
                  </>
                )}
              </Card.Subtitle>
            </Card.Body>
  ...
  const Research_national_List = Project_Research.National.map(
    (research, index) => {
      return (
        <>
          <Card key={index} bg="light" text="dark" style={{ width: "100%" }}>
  ...
  const Industry_National_List = Project_Industry.National.map(
    (industry, index) => {
      return (
        <>
          <Card key={index} bg="light" text="dark" style={{ width: "100%" }}>
            ...
            <Card.Body>
              <Card.Subtitle className="mb-2 text-muted">
                <span className="bold">
                  {industry.Funding_Agency.replace("+", "")}
                </span>
                {`, Rs: ${(industry.Cost_in_PKR / 1000000).toFixed(2)}M, `}
                {industry["Project_Status"] === "Completed" ? (
                  ...
                  <>
                    <span className="bold">
                      {industry["Project_Status"].substring(11)}
                    </span>
                    <span> ( {industry["Approval_Date"]} )</span>
                  </>
                )}
              </Card.Subtitle>
            </Card.Body>
  ...
  const Industry_International_List = Project_Industry.International.map(
    (industry, index) => {
      return (
        <>
          <Card key={index} bg="light" text="dark" style={{ width: "100%" }}>
  ...
  const IPS_Design = Intellectual_Property.Industrial_Design.map(
    (ip, index) => {
      return (
        <>
          <Card key={index} bg="light" text="dark" style={{ width: "100%" }}>
            ...
            <Card.Body>
              <Card.Subtitle className="mb-2 text-muted">
                <strong className={"strong-color"}>Status: </strong>{" "}
                {`${ip.Status} `}
                &nbsp; &nbsp; &nbsp;
  ...
  const qualifications = profile[0].Qualifications.map(
    (qualification, index) => {
      return (
        <Card key={index} bg="light" text="dark" style={{ width: "100%" }}>
          <Card.Header className={"project_header"}>
  ...
  const Trainings_Attended = profile[0].Trainings_Attended.map(
    (training, index) => {
      return (
        <>
          <Card key={index} bg="light" text="dark" style={{ width: "100%" }}>
    ...
    <>
          ...
          <div className="modal-container">
            <div className={modal ? "modal-barchart1" : "profile-charts-div"}>
              {Research_Articles.length +
                ...
                <PieChart
                  data={Publications_Pie_Chart_Date}
                  title={"Projects"}
                />
              )}
              {projects.length === 0 ? (
                ...
                <PieChart data={Project_Pie_Chart_Date} title={"Projects"} />
              )}
            </div>
            <div className={modal ? "modal-barchart1" : "profile-charts-div"}>
              {Research_Articles.length +
              ...
              {projects.length === 0 ? (
                ...
                <Line_Chart data={projects_data} title={"Projects"} />
              )}
            </div>
          </div>
        </>
      ...
      <div className={"User_Profile"} style={{}}>
            ...
            <div className={"Options"}>
              ...
              <div className={"profile"}>
                <Button
                  ...
                  aria-expanded={CollapseOptions.Profile_options}
                >
                  Profile
                  <h5>
                    {CollapseOptions.Profile_options ? (
              ...
              {projects.length === 0 ? (
                ...
                <div className={"Projects"}>
                  <Button
                    ...
                    aria-controls="Project-options-area"
                    ...
                    <div id={"Project-options-area"}>
                      {Project_Research.National.length === 0 &&
                        ...
                        <Button
                          ...
                          aria-controls={"research-project-options-area"}
                          aria-expanded={
                            CollapseOptions.research_project_options
                          }
                        >
                      ...
                      <Collapse className="collapse" in={CollapseOptions.research_project_options}>
                        <div id={"research-project-options-area"}>
                          {Project_Research.National.length === 0 ? (
                            ...
                            >
                              National{" "}
                              <span className={"number internal_national"}>
                                {Research_national_List.length}
                              </span>
                            </Button>
                          )}
                          {Project_Research.International.length === 0 ? (
                            ...
                            >
                              International{" "}
                              <span className={"internal_international"}>
                                {Research_international_List.length}
                              </span>
                            </Button>
                          )}
                  ...
                  <Collapse className="collapse" in={CollapseOptions.publications_options}>
                    <div id={"Publications-options-area"}>
                      {Research_Articles.length === 0 ? (
                        ...
                        >
                          Research Articles{" "}
                          <span className={"internal-articles"}>
                            {Research_Articles_List.length}
                          </span>
                        </Button>
                      )}
                      {conferences.length === 0 ? (
                        ...
                        >
                          ...
                          <span className={"internal-conference"}>
                            {Conferences.length}
                          </span>
                        </Button>
                      )}
                      {editorials.length === 0 ? (
                        ...
                        >
                          ...
                          <span className={"internal-editorials"}>
                            {Editorials.length}
                          </span>
                        </Button>
                      )}
                      {Books.length === 0 ? (
                        ...
                        >
                          Books{" "}
                          <span className={"internal-chapter internal-book"}>
                            {Books.length}
                          </span>
                        </Button>
                      )}
                      {Book_Chapters.length === 0 ? (
                        ...
                        >
                          Book Chapters{" "}
                          <span className={"internal-chapter"}>
                            {Book_Chapters.length}
                          </span>
                        </Button>
                      )}
                ...
                <div className={"IP"}>
                  <Button
                    onClick={() =>
                      setOptions({
                        ...CollapseOptions,
                        ip_options: !CollapseOptions.ip_options,
                      })
                    }
                    aria-controls="IP-options-area"
                  ...
                  <Collapse className="collapse" in={CollapseOptions.ip_options}>
                    <div id={"IP-options-area"}>
                      ...
                      {Intellectual_Property.Industrial_Design.length === 0 ? (
                        ""
                      ) : (
                        <Button
                          onClick={() => {
                        ...
                        >
                          Industrial Designs{" "}
                          <span className={"ips_ids"}>{IPS_Design.length}</span>
                        </Button>
                      )}
                      {Intellectual_Property.Copy_Rights.length === 0 ? (
                        ""
                      ) : (
                        <Button
                          onClick={() => {
              ...
              {Trainings.length + profile[0].Trainings_Attended.length === 0 ? (
                ""
              ) : (
                <div className={"trainings"}>
                  <Button
                    ...
                    aria-controls="trainings-options-area"
                    ...
                    <div id={"trainings-options-area"}>
                      {profile[0].Trainings_Attended.length === 0 ? (
                        ...
                        >
                          Attended{" "}
                          <span className={"trainings_number"}>
                            {profile[0].Trainings_Attended.length}
                          </span>
                        </Button>
                      )}
                      {Trainings.length === 0 ? (
                        ...
                        >
                          Conducted{" "}
                          <span className={"trainings_number-conducted"}>
                            {Trainings.length}
                          </span>
                        </Button>
                      )}
                  ...
                  <Collapse className="collapse" in={CollapseOptions.supervised_projects_options}>
                    <div id={"supervision-options-area"}>
                      {Supervision.PHD.length === 0 ? (
                        ...
                        >
                          Phd Supervision{" "}
                          <span className={"internal-supervisions"}>
                            {Doctoral.length}
                          </span>
                        </Button>
                      )}
                      {Supervision.Masters.length === 0 ? (
                        ""
                      ) : (
                        <Button
                          onClick={() => {
                        ...
                        >
                          MS Supervision{" "}
                          <span className={"internal-supervisions-ms"}>
                            {MS.length}
                          </span>
                        </Button>
                      )}
          ...
          <div className="">
            ...
            <div className={"faculty_personal_info"}>
              ...
              <div className={window.innerWidth < 910 ? "row" : "fonts"}>
                <div style={{ fontSize: "1em" }} className="details col-sm">
                  <FontAwesomeIcon icon={faGraduationCap} className={"font"} />
                  <h3 className={"designation"}>{profile[0].Work_Position}</h3>
                </div>
                <div style={{ fontSize: "1em" }} className="details col-sm">
                  <FontAwesomeIcon icon={faMapMarkerAlt} className={"font"} />
                  <h3 className={"department"}>{profile[0].School}</h3>
                </div>
              </div>
              <div className={window.innerWidth < 910 ? "row" : "fonts"}>
                <a
                  ...
                  href={
                    "mailto:" + profile[0].e_mail + "?cc=ddresearch@nust.edu.pk"
                  }
                >
                  Email Me
                </a>
                {/* <a className="emailMeButton" href={"mailto:" + profile[0].e_mail + "?cc=ddresearch@nust.edu.pk&subject=Hello%20from%20HTML&body=This%20is%20the%20body%20of%20the%20email."}>Email Me</a> */}
          ...
          <div className={"profile_links"}>
            <div className={"links"}>
              <ul className={"featured-Icons"}>
                <li className={"li"}>
                  <a
                    href={`${profile[0].linkedin_URL}`}
                    className={
                      profile[0].linkedin_URL.trim() === ""
                        ? "disabled_link"
                        : ""
                    }
                    target={"_blank"}
                ...
                <li className={"li"}>
                  <a
                    href={`${profile[0].twitter_URL}`}
                    className={
                      profile[0].twitter_URL.trim() === ""
                        ? "disabled_link"
                        : ""
                    }
                    target={"_blank"}
            ...
            <div className={"profile_links_1"}>
              <a
                ...
                className={ScopusID === "" ? "disabled" : "research-info"}
                target={"_blank"}
              >
                Scopus
              </a>
              <a
                href={`${profile[0].google_URl}`}
                className={
                  profile[0].google_URl.trim() === ""
                    ? "disabled"
                    : "research-info"
                }
                target={"_blank"}
            ...
            <div className={"profile_links_2"}>
              <button
                onClick={openCV}
                className={enable ? "CV-button" : "disabled"}
              >
                {" "}
                Download CV
              </button>
        ...
        <div className={"Main_Body"}>
              ...
              <div className={"analysis_tab_header"}>
                <div className={"about_heading"}>
                  ...
                  <h6 className={"introduction"}>
                    ...
                    <strong>{profile[0].School}</strong>, NUST.
                    <strong>Dr {profile[0].Name}</strong> has a PhD in{" "}
                    <strong>
                      {profile[0].Qualifications[0]["speciality"]}
                    </strong>
                    . &nbsp;<strong>Dr {profile[0].Name}</strong> has published{" "}
                    <strong>
                      {Research_Articles_List.length +
                        Books.length +
                        Book_Chapters.length +
                    ...
                    research articles & conference papers having a citation
                    count of <strong>{Citations}</strong>, carried out{" "}
                    <strong>
                      {Project_Research.National.length +
                        Project_Research.International.length +
                        Project_Industry.National.length +
          ...
          {!TabOptions.profile_tab ? (
            ...
            <div className={"Profile"}>
              {!profileData.Qualifications ? (
                ...
                <div className={"Profile_Qualification profile_field"}>
                  {qualifications}
                </div>
              )}
              ...
              {!profileData.Awards ? (
                ...
                <div className={"Profile_Awards profile_field"}>{awards}</div>
              )}
              {!profileData.Talks ? (
                ""
              ) : (
                <div className={"Profile_KeyNotes profile_field"}>
                  {keynotes}
                </div>
              )}
              {!profileData.Memberships ? (
                ...
                <div
                  className={"Profile_Professional_Membership profile_field"}
                >
          ...
          {!TabOptions.researchProjects_National_tab ? (
            ""
          ) : (
            <div className={"researchNational_Tab"}>
              <div className={"researchNational_Tab_header"}>
                {Research_national_List}
              </div>
            </div>
          )}
          {!TabOptions.researchProjects_International_tab ? (
            ""
          ) : (
            <div className={"researchInterNational_Tab"}>
              <div className={"researchInterNational_Tab_header"}>
                {Research_international_List}
              </div>
            </div>
          )}
          {!TabOptions.industrialProjects_National_tab ? (
            ""
          ) : (
            <div className={"industrialNational_Tab"}>
              <div className={"industrialNational_Tab_header"}>
                {Industry_National_List}
              </div>
            </div>
          )}
          {!TabOptions.industrialProjects_International_tab ? (
            ""
          ) : (
            <div className={"industrialInterNational_Tab"}>
              <div className={"industrialInterNational_Tab_header"}>
                {Industry_International_List}
              </div>
            </div>
          )}
          {!TabOptions.publications_Articles_tab ? (
            ""
          ) : (
            <div className={"publications_Articles_Tab"}>
              <div className={"publications_Articles_Tab_header"}>
                {Research_Articles_List}
              </div>
            </div>
          )}
          {!TabOptions.publications_Books_tab ? (
            ""
          ) : (
            <div className={"publications_Books_Tab"}>
              <div className={"publications_Books_Tab_header"}>{Book_List}</div>
            </div>
          )}
          {!TabOptions.publications_Chapters_tab ? (
            ""
          ) : (
            <div className={"publications_Chapters_Tab"}>
              <div className={"publications_Chapters_Tab_header"}>
                {Book_Chapters_List}
              </div>
            </div>
          )}
          {!TabOptions.Conference_tab ? (
            ""
          ) : (
            <div className={"Conference_Tab"}>
              <div className={"Conference_Tab_header"}>{Conferences}</div>
            </div>
          )}
          {!TabOptions.Editorial_Board_tab ? (
            ""
          ) : (
            <div className={"Editorial_Tab"}>
              <div className={"Editorial_Tab_header"}>{Editorials}</div>
            </div>
          )}
          {!TabOptions.Patents_International_tab ? (
            ""
          ) : (
            <div className={"Patents_International_Tab"}>
              <div className={"Patents_International_Tab_header"}>
                <h1 className={"Patents_International_Tab_header_text"}>
                  International Patents Tab
                </h1>
              </div>
            </div>
          )}
          {!TabOptions.Patents_National_tab ? (
            ""
          ) : (
            <div className={"Patents_National_Tab"}>
              <div className={"Patents_National_Tab_header"}>{IPS_Patent}</div>
            </div>
          )}
          {!TabOptions.Intellectual_Property_tab ? (
            ""
          ) : (
            <div className={"Intellectual_Property_Tab"}>
              <div className={"Intellectual_Property_Tab_header"}>
                <h1 className={"Intellectual_Property_Tab_header_text"}>
                  Intellectual Property Tab
                </h1>
              </div>
            </div>
          )}
          {!TabOptions.Industrial_Design_tab ? (
            ""
          ) : (
            <div className={"Intellectual_Property_ID_Tab"}>
              <div className={"Intellectual_Property_ID_Tab_header"}>
                {IPS_Design}
              </div>
            </div>
          )}
          {!TabOptions.Trade_Marks_tab ? (
            ""
          ) : (
            <div className={"TradeMark_Tab"}>
              <div className={"TradeMark_Tab_header"}>{IPS_TradeMark}</div>
            </div>
          )}
          {!TabOptions.Copyright_tab ? (
            ""
          ) : (
          ...
          {!TabOptions.Training_Attended_tab ? (
            ...
            <div className={"Training_Attended_Tab"}>
              <div>{Trainings_Attended}</div>
            </div>
          )}
          {!TabOptions.Training_Conducted_tab ? (
            ""
          ) : (
            <div className={"Training_Conducted_Tab"}>
              <div className={"Training_Conducted_Tab_header"}>{Trainings}</div>
            </div>
          )}
          {!TabOptions.Supervision_Masters_tab ? (
            ""
          ) : (
            <div className={"Supervision_Masters_Tab"}>
              <div className={"Supervision_Masters_Tab_header"}>{MS}</div>
            </div>
          )}
          {!TabOptions.Supervision_PHD_tab ? (
            ""
          ) : (
            <div className={"Supervision_PHD_Tab"}>
              <div className={"Supervision_PHD_Tab_header"}>{Doctoral}</div>
            </div>
          )}
        </div>
      </div>
